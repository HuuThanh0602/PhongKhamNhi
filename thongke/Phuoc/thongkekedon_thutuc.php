...
try {
    ...
    if ($maLoaiThuocInput) {
        // Sử dụng câu lệnh gọi thủ tục với cú pháp chuẩn cho SQL Server
        $sql = "EXEC ProcThongKeKeDon @MaLoaiThuocInput = :MaLoaiThuocInput";
...
<nav class="navbar navbar-expand-lg navbar-dark bg-dark">
    <div class="container">
        ...
        <div><a href="../ThongKe.php">Quay Lại</a></div>
    </div>
</nav>
...
<div class="container">
    <?php if ($hasResult) { ?>
        <table class="table table-bordered table-striped">
            ...
            <tbody>
                <?php 
                // Lấy dữ liệu từ thủ tục khi đã có mã loại thuốc
                $stmt = $conn->prepare("EXEC ProcThongKeKeDon @MaLoaiThuocInput = :MaLoaiThuocInput");
                $stmt->bindParam(':MaLoaiThuocInput', $maLoaiThuocInput, PDO::PARAM_STR);
                $stmt->execute();

                // Lặp qua kết quả và hiển thị thông tin
                while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) { 
                    // Kiểm tra nếu TongSoLuongThuoc là NULL, thay thế thành 0
                ...
                ?>
                    <tr>
                        ...
                        <td><?php echo htmlspecialchars($tongSoLuongThuoc); ?></td>
                    </tr>
                <?php }
                // Đóng kết nối
                $stmt = null;
